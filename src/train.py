...
def train(model, train_dataset, test_dataset, device, 
    ...
    # Save initial model
    torch.save(model.state_dict(), 'model/initial_model.pth')
    ...
    elif os.path.exists('model/initial_model.pth'):
        model.load_state_dict(torch.load('model/initial_model.pth'))
